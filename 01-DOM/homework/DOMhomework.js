...
creador.innerHTML=creador.innerHTML + " Andrés Torreira";
...
function buildToDo(todo, index) {
  ...
  //toDoShell = document.addclass("toDoShell");
  toDoShell.setAttribute ('class','toDoShell');
  // = document.getElementsByClassName('toDosShell');
  var toDoText= document.createElement('span');
  toDoText.innerHTML=todo.description;
  toDoText.setAttribute('id',index);
  //toDoText.id=index;
  if (todo.complete){
    //toDoText=document.getElementsByClassName('completeText');
...
function buildToDos(toDos) {
  // Tu código acá:
var arraytodos=[];
//arraytodos=toDos.map(  buildToDo([toDos],0))
arraytodos=toDos.map(function(todo,index){return buildToDo(todo,index)});
return arraytodos;
}
...
function displayToDos() {
  ...
  for (let x = 0; x < arr.length; x++) {
    toDoContainer.appendChild((arr[x]));
...
function addToDo() {
  // Tu código acá:
  //tambien puedo acceder asi document.getElementById('toDoInput').value
